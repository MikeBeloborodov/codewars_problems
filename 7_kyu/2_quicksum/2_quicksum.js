...
const quicksum = (packet) => {
  ...
  for (let i = 0; i < packetArray.length; i++) {
    const flag =
      packetArray[i] === " "
        ? 1
        : packetArray[i].toLowerCase() === packetArray[i]
        ? 0
        : -1;
    if (flag === 0) return 0;
    if (flag === 1) continue;
    result += (i + 1) * (packetArray[i].charCodeAt() - 64);
  }
  return result;
};

console.assert(quicksum("ACM") === 46, '"ACM" === 46');
console.assert(quicksum("A C M") === 75, '"A C M" === 75');
console.assert(quicksum("AbqTH") === 0, '"AbqTH" === 0');
