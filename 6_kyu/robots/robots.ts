/*
  https://www.codewars.com/kata/587ae98e2ab0ef32ef00004c/train/typescript
...
export const countRobots = (a: string[]) => {
  const robotRegExp =
    /[a-z][|};&#\[\]\/><()\*]{2}0[|};&#\[\]\/><()\*]{2}0[|};&#\[\]\/><()\*]{2}[a-z]/gi;
  let automatik: number = 0;
  let mechanik: number = 0;

  a.forEach((str) => {
    const robotsMatches = str.match(robotRegExp);
    if (robotsMatches) {
      if (/automatik/i.test(str)) automatik += robotsMatches.length;
      if (/mechanik/i.test(str)) mechanik += robotsMatches.length;
    }
  });
